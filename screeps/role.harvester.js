let roleHarvester = {
    run: function (creep, resources, bigresources, spawneng, towereng, storage, container, terminal, take_over_link, died_resource) {
        ...
        if (creep.memory.moving == 0) {
            if (creep.store.getFreeCapacity() == creep.store.getCapacity()) {
                ...
                if(container != '') {
                    ...
                    return object.store.getUsedCapacity(RESOURCE_ENERGY) > creep.store.getCapacity();
                    });
                }
                ...
                if (spawneng != '') {
                    ...
                    if (Memory.creeps[`${creep.room.name}_Transfer000`] == undefined && terminal.store[RESOURCE_ENERGY] < 30000){
                        creep.memory.target = terminal.id;
                        creep.memory.doing = 't';
                        creep.memory.moving = 1;
                    } else if (storage != '' && (died_resource != '' || resources != '' || take_over_link != '')) {
                        creep.memory.target = storage[0].id;
                        creep.memory.doing = 't';
                        creep.memory.moving = 1;
                    }
